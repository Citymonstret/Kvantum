...
public class SyntaxHandler extends WorkerProcedure.StringHandler
{
    ...
    private final Map<String, JtwigTemplate> templateStorage;

    SyntaxHandler()
    {
        this.configuration = new DefaultEnvironmentConfiguration();
        this.templateStorage = new HashMap<>();
    }
    ...
    public String act(final RequestHandler requestHandler, final Request request, final String in)
    {
        ...
        if ( !( requestHandler instanceof IgnoreSyntax ) )
        {
            if ( CoreConfig.debug )
            {
                Message.TEMPLATING_ENGINE_REACTING.log( "JTwigEngine", request );
            }
