...
@SuppressWarnings({"unused", "WeakerAccess"}) public class Path {
    ...
    public void registerWatcher(final FileWatcher fileWatcher,
        final BiConsumer<Path, WatchEvent.Kind<?>> reaction) throws IOException {
        if (!this.isFolder()) {
    ...
    final public byte[] readBytes() {
        ...
        final Optional<CachedFile> cacheEntry =
            fileSystem.getFileCacheManager().readCachedFile(this);
    ...
    public boolean copy(final Path targetDirectory) {
        final Path target =
            targetDirectory.forcePath(this.getEntityName() + "." + this.getExtension());
        if (this.isFolder()) {
    ...
    public Collection<Path> getSubPaths(boolean includeFolders) {
        ...
        return subPaths.values().stream().filter(path1 -> !path1.isFolder)
            .collect(Collectors.toList());
    }

    @Override public boolean equals(final Object o) {
        return this == o || (o != null && getClass() == o.getClass() && path
            .equals(((Path) o).path));
    }
