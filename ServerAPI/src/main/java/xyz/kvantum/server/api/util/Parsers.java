...
@SuppressWarnings({ "WeakerAccess", "unused" }) @UtilityClass final public class Parsers
{
	private static final Map<Type, Parser<?>> primitiveParserMap = MapBuilder.<Type, Parser<?>>newHashMap()
			.put( int.class, new IntegerParser() ).put( String.class, new StringParser() )
			.put( byte.class, new ByteParser() ).put( char.class, new CharParser() )
			.put( short.class, new ShortParser() ).put( long.class, new LongParser() )
			.put( float.class, new FloatParser() ).put( double.class, new DoubleParser() )
			...
			if ( this.hasRange && value != null )
			{
				...
				if ( b != value )
				{
					return new ParserResult<>(
							value + " is not within range (" + this.min + " -> " + ( this.max == Long.MAX_VALUE
									? "infinity"
									: this.max ) + ")" );
				}
			}
	...
	public static final class FloatParser extends Parser<Float>
	{
		...
		@Override public ParserResult<Float> parse(final String in)
		{
			...
			if ( this.hasRange && value != null )
			{
				...
				{
					return new ParserResult<>(
							value + " is not within range (" + this.min + " -> " + ( this.max == Float.MAX_VALUE
									? "infinity"
									: this.max ) + ")" );
				}
			}
	...
	public static final class DoubleParser extends Parser<Double>
	{
		...
		@Override public ParserResult<Double> parse(final String in)
		{
			...
			if ( this.hasRange && value != null )
			{
				...
				{
					return new ParserResult<>(
							value + " is not within range (" + this.min + " -> " + ( this.max == Double.MAX_VALUE
									? "infinity"
									: this.max ) + ")" );
				}
			}
