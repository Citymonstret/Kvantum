...
/**
 * Common {@link Parser} utilities and implementations. Does not provide implementations already existing in Commands:
 * <p> <ul> <li>For integer, see {@link IntegerParser}</li> <li>For String, see {@link StringParser}</li> <li>For
 * boolean, see {@link BooleanParser}</li> </ul> </p>
 */
@SuppressWarnings({"WeakerAccess", "unused"}) @UtilityClass final public class Parsers {
    private static final Map<Type, Parser<?>> primitiveParserMap =
        MapBuilder.<Type, Parser<?>>newHashMap().put(int.class, new IntegerParser())
            .put(String.class, new StringParser()).put(byte.class, new ByteParser())
            .put(char.class, new CharParser()).put(short.class, new ShortParser())
            .put(long.class, new LongParser()).put(float.class, new FloatParser())
            .put(double.class, new DoubleParser()).put(boolean.class, new BooleanParser()).get();
            ...
            if (this.hasRange && value != null) {
                ...
                if (b != value) {
                    return new ParserResult<>(
                        value + " is not within range (" + this.min + " -> " + this.max + ")");
                }
            }

            return value != null ?
                new ParserResult<>(value) :
                new ParserResult<>(in + " is not a byte");
        }
    }
    ...
    public static final class CharParser extends Parser<Character> {
        ...
        @Override public ParserResult<Character> parse(final String in) {
            ...
            return value != null ?
                new ParserResult<>(value) :
                new ParserResult<>(in + " is not a character");
        }
    }
    ...
    public static final class ShortParser extends Parser<Short> {
        ...
        @Override public ParserResult<Short> parse(final String in) {
            ...
            return value != null ?
                new ParserResult<>(value) :
                new ParserResult<>(in + " is not a short");
        }
    }
    ...
    public static final class LongParser extends Parser<Long> {
        ...
        @Override public ParserResult<Long> parse(final String in) {
            ...
            if (this.hasRange && value != null) {
                ...
                if (b != value) {
                    return new ParserResult<>(
                        value + " is not within range (" + this.min + " -> " + (
                            this.max == Long.MAX_VALUE ? "infinity" : this.max) + ")");
                }
            }

            return value != null ?
                new ParserResult<>(value) :
                new ParserResult<>(in + " is not a long");
        }
    }
        ...
        @Override public ParserResult<Float> parse(final String in) {
            ...
            if (this.hasRange && value != null) {
                ...
                if (b != value) {
                    return new ParserResult<>(
                        value + " is not within range (" + this.min + " -> " + (
                            this.max == Float.MAX_VALUE ? "infinity" : this.max) + ")");
                }
            }

            return value != null ?
                new ParserResult<>(value) :
                new ParserResult<>(in + " is not a float");
        }
    }
        ...
        @Override public ParserResult<Double> parse(final String in) {
            ...
            if (this.hasRange && value != null) {
                ...
                if (b != value) {
                    return new ParserResult<>(
                        value + " is not within range (" + this.min + " -> " + (
                            this.max == Double.MAX_VALUE ? "infinity" : this.max) + ")");
                }
            }

            return value != null ?
                new ParserResult<>(value) :
                new ParserResult<>(in + " is not a double");
        }
    }
