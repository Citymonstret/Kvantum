 ...
 * they are handled.
 */
@EqualsAndHashCode(of = "uniqueId") @SuppressWarnings({"WeakerAccess", "unused"})
public abstract class RequestHandler {

    private static final Class[] REQUIRED_PARAMETERS =
     ...
     */
    @Getter private final MiddlewareQueuePopulator middlewareQueuePopulator =
        new MiddlewareQueuePopulator();
    ...
    @Getter private long matchCount = 0L;
    ...
    public final Response handle(final AbstractRequest request) {
        ...
        if (!middlewareQueue.finished()) {
            ServerImplementation.getImplementation()
                .log("Skipping request as a middleware broke the chain!");
            ...
            if (method.isPresent()) {
                ...
                throw new KvantumException(
                    "Trying to access an internal redirect which isn't registered for type " + this
                        .getName() + ", identified by " + request
                        .getMeta(AbstractRequest.ALTERNATE_OUTCOME));
            }
        } else {
            //
        ...
        if (response == null) {
            throw new KvantumException(
                "ResponseHandler (" + this.getName() + ") generated a null response");
        }
