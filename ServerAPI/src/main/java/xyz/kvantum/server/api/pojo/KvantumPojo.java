...
/**
 * Representation of a POJO instance constructed by a {@link KvantumPojoFactory} instance <p> Delegates {@link
 * #toString()}, {@link #equals(Object)} and {@link #hashCode()} to the POJO instance
 */
@RequiredArgsConstructor(access = AccessLevel.PACKAGE) @SuppressWarnings({ "unused",
		"WeakerAccess" }) public final class KvantumPojo<Pojo> implements VariableProvider, JSONAware
{
	...
	/**
	 * Update a field by accessing a setter in the POJO instance. Following the name pattern "setField"
	 *
		...
		if ( this.fieldSetters.containsKey( field ) )
		{
			...
			if ( !value.getClass().isAssignableFrom( setter.getParameterType() ) )
			{
				throw new IllegalArgumentException(
						String.format( "Provided '%s', expected '%s'", value.getClass().getSimpleName(),
								setter.getParameterType().getSimpleName() ) );
			}
