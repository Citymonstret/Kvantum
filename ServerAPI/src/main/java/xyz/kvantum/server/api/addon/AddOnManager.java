...
@SuppressWarnings("unused") public final class AddOnManager extends AutoCloseable {
    ...
    @Nullable private Properties getAddOnProperties(@NonNull final File file)
        ...
        try {
            ...
            throw new AddOnManagerException("Failed to create jar object from " + file.getName(),
                e);
        }
        ...
        try (final InputStream stream = jar.getInputStream(desc)) {
            ...
            throw new AddOnManagerException(
                "Failed to load \"addon.properties\" in " + file.getName());
        }
        return properties;
    }
    ...
    @SuppressWarnings("WeakerAccess") public <T extends AddOn> void unloadAddon(
        ...
        if (!this.classLoaders.containsKey(name)) {
            throw new AddOnManagerException(
                "Cannot find class loader by name \"" + name + "\". Panicking!");
        }
