 ...
 * @author Citymonstret
 */
@SuppressWarnings("unused") @NoArgsConstructor(access = AccessLevel.PROTECTED)
public abstract class AbstractRequest
    implements ProviderFactory<AbstractRequest>, VariableProvider, Validatable, RequestChild,
    VariableHolder {
    ...
    @Getter private final Map<String, ProviderFactory<? extends VariableProvider>> models =
        new HashMap<>();
    ...
    public String buildLog() {
        ...
        for (final Object a : new CharSequence[] {socket.getAddress().toString(),
            getHeader("User-Agent"), getHeader("query"), getHeader("Host"),
            this.query.buildLog()}) {
            ...
            if (resourceName.contains("?")) {
                ...
                if (parts.length > 1) {
                    ...
                    for (final String part : subParts) {
                        ...
                        this.parameters
                            .put(subSubParts[0], subSubParts.length > 1 ? subSubParts[1] : "");
                    }
                } else {
    ...
    @SuppressWarnings("unused") public final static class Authorization {
        ...
        Authorization(@Nonnull @NonNull final AsciiString input) {
            ...
            final val auth =
                AsciiString.of(Base64.getDecoder().decode(parts.get(2).getValue())).split(":");
            if (auth.size() < 2) {
                this.username = null;
                this.password = null;
            } else {
