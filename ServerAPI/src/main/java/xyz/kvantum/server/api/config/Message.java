 ...
 */
public enum Message {CREATING("Creating {}...", LogLevels.LEVEL_INFO), WAITING_FOR_EXECUTOR_SERVICE(
    "Waiting for the executor service to shutdown", LogLevels.LEVEL_INFO), TEMPLATING_ENGINE_STATUS(
    "Templating language '{0}': {1}", LogLevels.LEVEL_INFO), TEMPLATING_ENGINE_REACTING(
    "{0} is reacting to: {1}", LogLevels.LEVEL_DEBUG), TEMPLATING_ENGINE_DEBUG_NOT_ENABLED(
    ...
    LogLevels.LEVEL_INFO), MD5_DIGEST_NOT_FOUND("Could not load the MD5 MessageDigest Instance {}",
    LogLevels.LEVEL_ERROR), CLIENT_NOT_ACCEPTING_GZIP("The client does not accept GZIP encoding {}",
    ...
    LogLevels.LEVEL_INFO), OUTPUT_BUFFER_INFO("Output buffer size: {0}kb | Input buffer size: {1}kb",
    LogLevels.LEVEL_DEBUG), ACCEPTING_CONNECTIONS_ON("Accepting connections on 'http://{}",
    LogLevels.LEVEL_INFO), ACCEPTING_SSL_CONNECTIONS_ON("Accepting SSL connections on 'https://{}'",
    LogLevels.LEVEL_INFO), SHUTTING_DOWN("Shutting down", LogLevels.LEVEL_INFO), STARTING_ON_PORT(
    ...
    LogLevels.LEVEL_INFO), SERVER_STARTED("The server is started", LogLevels.LEVEL_INFO), TICK_ERROR(
    "Error in server ticking...", LogLevels.LEVEL_ERROR), REQUEST_LOG(
    ...
    LogLevels.LEVEL_DEBUG), CONNECTION_ACCEPTED("Connection accepted from '{}' - Handling the data!",
    LogLevels.LEVEL_DEBUG), DEBUG(">> Debug - Ignore <<",
    LogLevels.LEVEL_DEBUG), CANNOT_LOAD_TRANSLATIONS("Cannot load the translation file",
    LogLevels.LEVEL_ERROR), APPLICATION_CANNOT_FIND(
    ...
    "Loading views...", LogLevels.LEVEL_INFO), INTERNAL_REDIRECT("Redirecting request to \"/{}\"",
    LogLevels.LEVEL_DEBUG), VIEWS_DISABLED("Skipped view loading (Disabled)",
    LogLevels.LEVEL_INFO), STARTUP_STEP("Calling Startup Step: '{}'",
    LogLevels.LEVEL_DEBUG), CACHING_DISABLED(
    ...
    LogLevels.LEVEL_INFO), CLEARED_VIEWS("Cleared views ( {} )", LogLevels.LEVEL_INFO), SESSION_SET(
    "Set session ({0}={1})", LogLevels.LEVEL_DEBUG), SESSION_FOUND(
    "Found session ({0}={1}) for request {2}", LogLevels.LEVEL_DEBUG), SESSION_DELETED_OUTDATED(
    "Deleted outdated session: {}", LogLevels.LEVEL_DEBUG), SESSION_DELETED_OTHER(
    "Deleted session: {} (Cause: {})", LogLevels.LEVEL_DEBUG), SESSION_KEY_INVALID(
    "Wrong session key", LogLevels.LEVEL_INFO), CMD_HELP_HEADER(
    ...
    "Created admin account with password: \"{}\"", LogLevels.LEVEL_INFO), WORKER_FAILED_HANDLING(
    "Error when handling request: {}", LogLevels.LEVEL_ERROR), SERVER_START_FAILED(
    "Failed to start server..."), SERVER_START_PORT_CHANGED_PRIVILEGED(
    "Failed to bind to privileged port, trying 1024 instead",
    LogLevels.LEVEL_ERROR), SERVER_START_PORT_CHANGED_OCCUPIED("Port {0} is occupied. Trying {1}...",
    LogLevels.LEVEL_ERROR), REQUEST_HANDLER_DUMP("> RequestHandler - Class '{0}', Pattern: '{1}'",
