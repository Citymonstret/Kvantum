...
class ExampleSearch
{
	...
	ExampleSearch()
	{
		...
		//      ]
		// }
		KvantumSearchService.<Account, IAccount>builder().filter( "search" ).queryObjectType( Account.class )
				.resultProvider(
						ServerImplementation.getImplementation().getApplicationStructure().getAccountManager() )
				.matcher( new AccountMatcherFactory<>() ).parameterScope( ParameterScope.GET ).build().createService();
		...
		//      ]
		// }
		RSQLSearchService.<IAccount>builder().filter( "rsqlsearch" ).parameterScope( ParameterScope.GET )
				.resultProvider(
						ServerImplementation.getImplementation().getApplicationStructure().getAccountManager() )
