...
public class ExampleAccountRegistration {
    ...
    //
    private final KvantumObjectFactory<Account> kvantumObjectFactory =
    ...
    //
    private final KvantumPojoFactory<Account> kvantumPojoFactory =
        KvantumPojoFactory.forClass(Account.class);
    ...
    @ViewMatcher(filter = "register", httpMethod = HttpMethod.POST, outputType = StandardConverters.JSON)
    @SuppressWarnings("unused") public JSONObject onRegister(final AbstractRequest request) {
        final JSONObject output = new JSONObject();
        //
        // We make sure that the client has provided all required parameters
        //
        final RequestRequirements.RequirementStatus requirementStatus =
            requestRequirements.testRequirements(request);
        if (!requirementStatus.passed()) {
            ...
            //
            final KvantumObjectParserResult<Account> kvantumObjectParserResult =
                kvantumObjectFactory.build(ParameterScope.POST).parseRequest(request);
            if (!kvantumObjectParserResult.isSuccess()) {
                ...
                if (!brokenRules.isEmpty()) {
                    ...
                    if (!accountManager.findAllByQuery(comparator).isEmpty()) {
                        ...
                        //
                        final Optional<IAccount> accountCreationOptional =
