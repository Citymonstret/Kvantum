...
@SuppressWarnings("ALL") final public class KvantumMain
{
	...
	public static void main(final String[] args) throws Throwable
	{
		...
		if ( options.help )
		{
			...
			logWrapper.log( LogContext.builder().applicationPrefix( "Help" ).logPrefix( "Info" )
					.timeStamp( TimeUtil.getTimeStamp() ).message( message.toString() ).thread( "main" ).build() );
		...
		} else
		{
			...
			final Optional<Kvantum> server = ServerContext.builder().coreFolder( file ).standalone( true )
	...
	private static class Options
	{
		...
		@Parameter(names = "-folder", description = "Application base folder path") private String folder = "";
