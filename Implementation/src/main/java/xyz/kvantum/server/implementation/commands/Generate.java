...
@CommandDeclaration(command = "generate", usage = "/generate <folder> <base uri path>", description = "See: https://github.com/IntellectualSites/Kvantum/wiki/generate")
public class Generate extends Command {

    public Generate() {
        this.withArgument("folder", new StringParser(), "Starting folder");
        this.withArgument("basePath", new StringParser(),
            "URI path (use none for default folder path)");
    }
    ...
    @Override public boolean onCommand(final CommandInstance instance) {
        ...
        final Path path =
            ServerImplementation.getImplementation().getFileSystem().getPath(folderName);
        if (!path.exists()) {
        ...
        try {
            configurationFile = new YamlConfiguration(fileName, new File(
