...
@RequiredArgsConstructor final class KvantumServerHandler extends ChannelInboundHandlerAdapter {
    ...
    // Metrics
    //
    static final Timer TIMER_WRITE_RESPONSE =
    ...
    static final Timer TIMER_ROUTING =
        ServerImplementation.getImplementation().getMetrics().getRegistry()
            .timer(MetricRegistry.name(KvantumServerHandler.class, "routing"));
    static final Timer TIMER_WRITE_TO_CLIENT =
        ServerImplementation.getImplementation().getMetrics().getRegistry()
            .timer(MetricRegistry.name(KvantumServerHandler.class, "writeToClient"));
    static final Timer TIMER_READ_BYTES =
        ServerImplementation.getImplementation().getMetrics().getRegistry()
            .timer(MetricRegistry.name(KvantumServerHandler.class, "readBytes"));
    ...
    @Override public void handlerAdded(final ChannelHandlerContext context) {
        ...
        final Supplier<Boolean> activeCheck =
            () -> context.channel().isOpen() && context.channel().isActive();
    ...
    @Override public void channelActive(final ChannelHandlerContext ctx) throws Exception {
        ...
        //
        final ConnectionEstablishedEvent connectionEstablishedEvent =
            new ConnectionEstablishedEvent(this.workerContext.getSocketContext().getIP());
        ServerImplementation.getImplementation().getEventBus()
            .throwEvent(connectionEstablishedEvent, false);
        //
    ...
    public void exceptionCaught(final ChannelHandlerContext context, final Throwable cause) {
        if (cause instanceof ReadTimeoutException) {
            if (CoreConfig.debug) {
                Logger
                    .debug("Connection for {} timed out", workerContext.getSocketContext().getIP());
            }
        } else {
