...
public class SimpleServer implements Kvantum
{
	 ...
	 */
	@SneakyThrows @SuppressWarnings("WeakerAccess") public SimpleServer(@NonNull final ServerContext serverContext)
	{
		...
		if ( CoreConfig.Sessions.enableDb )
		{
			...
			case "sqlite":
				sessionDatabase = new SQLiteSessionDatabase(
						( SQLiteApplicationStructure ) this.getApplicationStructure().getAccountManager()
								.getApplicationStructure() );
				break;
			case "mongo":
				sessionDatabase = new MongoSessionDatabase(
						( MongoApplicationStructure ) this.getApplicationStructure().getAccountManager()
								.getApplicationStructure() );
				break;
			case "mysql":
				sessionDatabase = new MySQLSessionDatabase(
						( MySQLApplicationStructure ) this.getApplicationStructure().getAccountManager()
								.getApplicationStructure() );
				break;
			default:
	 ...
	 */
	@Synchronized private void log(@NonNull final String prefix, @NonNull final String message, final Object... args)
	{
		String msg = message;
		...
		getLogWrapper().log( LogContext.builder().applicationPrefix( CoreConfig.logPrefix ).logPrefix( prefix )
				.timeStamp( TimeUtil.getTimeStamp() ).message( msg ).thread( Thread.currentThread().getName() )
				.build() );
	}
	...
	@Override public final void log(final LogProvider provider, final String message, final Object... args)
	{
