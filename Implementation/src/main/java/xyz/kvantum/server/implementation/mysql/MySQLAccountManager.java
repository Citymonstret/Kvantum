...
public class MySQLAccountManager implements IAccountManager
{
    ...
    {
        this.applicationStructure.getDatabaseManager().executeUpdate( "CREATE TABLE IF NOT EXISTS `account` (" +
                "  `id` int(11) NOT NULL AUTO_INCREMENT," +
                "  `username` varchar(64) DEFAULT NULL," +
                "  `password` varchar(255) DEFAULT NULL," +
                "  PRIMARY KEY (`id`)," +
                "  UNIQUE KEY `accounts_UN` (`username`) )" );
        this.applicationStructure.getDatabaseManager().executeUpdate( "CREATE TABLE IF NOT EXISTS `account_data` (" +
                " `id` int(11) NOT NULL AUTO_INCREMENT," +
                " `account_id` int(11)," +
                " `key` VARCHAR(255)," +
                " `value` VARCHAR(255)," +
                " PRIMARY KEY (`id`)," +
                " UNIQUE KEY `account_data_UN` (`account_id`, `key`) )" );
        this.checkAdmin();
    }
    ...
    public Optional<IAccount> createAccount(@NonNull final IAccount temporary)
    {
        ...
        {
            try ( final PreparedStatement statement = connection.prepareStatement( "INSERT INTO account(`username`, " +
                    "`password`) VALUES(?, ?)" ) )
            {
    ...
    public Optional<IAccount> getAccount(final int accountId)
    {
        ...
        try ( final Connection connection = applicationStructure.getDatabaseManager().getConnection() )
        {
            try ( final PreparedStatement statement = connection.prepareStatement( "SELECT * FROM `account` WHERE `id` = ?"
            ) )
            {
    ...
    public void deleteAccount(@NonNull final IAccount account)
    {
        try ( final Connection connection = applicationStructure.getDatabaseManager().getConnection() )
        {
            try ( final PreparedStatement statement = connection.prepareStatement( "DELETE FROM `account` WHERE `id` = ?" ) )
            {
                statement.setInt( 1, account.getId() );
                statement.executeUpdate();
