...
@SuppressWarnings({"unused", "WeakerAccess"}) public class AccountVerifier {
    ...
    public static final Rule<IAccount> USERNAME_MIN_5_CHARACTERS = PredicatedRule
        .create("Username must be at least 5 characters long",
            account -> account.getUsername().length() >= 5);
    public static final Rule<IAccount> EMAIL_PATTERN =
        PredicatedRule.create("Email must follow valid email " + "syntax", account -> {
    ...
    public AccountVerifier() {
        ...
        builder.nullable(false).withRule(PASSWORD_MIN_CHARACTERS).withRule(PASSWORD_MAX_CHARACTERS)
            .withRule(PASSWORD_NOT_USERNAME).withRule(USERNAME_MIN_5_CHARACTERS)
